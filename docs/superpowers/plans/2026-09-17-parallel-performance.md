...
- [ ] **Step 3: Implement the minimal context and bin types.**
...
- [ ] **Step 5: Add the test to the Makefile and commit.**
...
- [ ] **Step 1: Add the worker-context API declarations and compile-failing call sites.**
...
- [ ] **Step 2: Run `make -j2` and record the expected compile failures.**
...
- [ ] **Step 3: Move the existing collection arithmetic into the supplied-bin overload.**
...
- [ ] **Step 4: Implement context creation and reduction.**
...
- [ ] **Step 5: Implement context report overloads without common-path locks.**
...
- [ ] **Step 6: Run the existing native suite and commit.**
...
- [ ] **Step 1: Add a seeded process-level regression command before wiring the worker.**
...
- [ ] **Step 3: Add context-aware event generation and propagation.**
...
- [ ] **Step 4: Replace one-phonon scheduling with chunked index allocation.**
...
- [ ] **Step 5: Add the regression to `make test` and run it.**
...
- [ ] **Step 1: Add documentation for worker-local reduction.**
...
- [ ] **Step 4: Run the matched performance benchmark.**
...
- [ ] **Step 5: Review the diff and commit the documentation.**
...
- [ ] **Step 1: Run the complete verification set on the final tree.**

Run: `make -j2`, `make test`, and `make test-plotting` when optional plotting dependencies are installed.

- [ ] **Step 2: Confirm the worktree and branch state.**

Run: `git status --short --branch` and `git log --oneline --decorate -8`. The worktree must be clean on `parallel-performance`; generated benchmark data must remain ignored.

- [ ] **Step 3: Report the implementation and measured result.**

Include the branch name, commit identifiers, tests run, benchmark configuration, serial/parallel elapsed times, speedups, any numerical caveat, and the isolated worktree path.
